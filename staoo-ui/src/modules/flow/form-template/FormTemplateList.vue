<template>
  <div class="form-template-list">
    ...
    <el-table v-loading="loading" :data="formTemplateList" style="width: 100%">
      ...
      <el-table-column prop="formKey" label="表单标识" width="180" />
...
import { getFormTemplateList, publishFormTemplate, deleteFormTemplate, copyFormTemplate } from '../../../services/flow/formTemplateService'
...
const loadFormTemplateList = async () => {
  ...
  try {
    ...
    const response = await getFormTemplateList(params)
    if (response.code === 200 && response.data) {
      formTemplateList.value = response.data.list
...
// 新建
const handleCreate = () => {
  router.push('/flow/form-template/edit')
}
...
const handlePublish = async (id: number) => {
  try {
    ElMessageBox.confirm('确定要发布该表单模板吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      const response = await publishFormTemplate(id)
...
const handleDelete = async (id: number) => {
  try {
    ElMessageBox.confirm('确定要删除该表单模板吗？删除后将无法恢复！', '警告', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'error'
    }).then(async () => {
