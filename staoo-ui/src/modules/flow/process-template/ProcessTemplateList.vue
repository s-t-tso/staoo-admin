...
const handleEdit = (list: any) => {
  router.push(`/flow/process-template/edit?id=${list.id}`)
}
...
const handleCopy = async (list: any) => {
  try {
    const newName = await ElMessageBox.prompt(`请输入新流程名称（原名称：${list.processName}）`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
    ...
    if (newName.value) {
      const response = await copyProcessTemplate({ id: list.id, newName: newName.value })
      if (response.code === 200) {
